...
f=$(basename $1)
if [[ $2 == "2" ]]; then
	cat $1 | egrep -o -e '. \[[^]]*\]' -e '\[[^]]*\] .'  | sort -f | uniq -c | sort -gr  | gsed 's/^ *//g' | gsed 's/ /@/1' > /tmp/${f}.patterns
elif [[ $2 == "3" ]]; then
	cat $1 | egrep -o -e '. \[[^]]*\] .' | sort -f | uniq -c | sort -gr  | gsed 's/^ *//g' | gsed 's/ /@/1' > /tmp/${f}.patterns
elif [[ $2 == "1" ]]; then
	cat $1 | egrep -o -e '\[[^]]*\]' | sort -f | uniq -c | sort -gr  | gsed 's/^ *//g' | gsed 's/ /@/1' > /tmp/${f}.patterns
else
...
for i in `cat /tmp/${f}.patterns | tr ' ' '_'`; do 
	w=`echo $i | cut -f2 -d'@' | tr '_' ' ' | gsed 's/\[/\\\[/g' | gsed 's/\]/\\\]/g'`; 
	cat ${f} | grep "$w" | awk -F'\t' '{print $2"\t"$1}' | sort -n | head -1 | gsed "s/^/$i\t/g" | tr '_' ' ' | gsed 's/@/\t/g' ;
done
